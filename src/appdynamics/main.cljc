(ns appdynamics.main
  "Kotodama WASM entrypoint for the Appdynamics clean-room actor (L4) — Clojure port.
  ...
  facts (`appdynamics.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "appdynamics")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"appdynamics.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "appdynamics-compat" :tier tier :entities entities} 200])
...
(defn start! [] :appdynamics-compat/ready)
